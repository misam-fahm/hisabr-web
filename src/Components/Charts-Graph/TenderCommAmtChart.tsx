...
const TenderCommAmtChart = ({ tenderData }: any) => {
  const [isMobile, setIsMobile] = useState<boolean>(false);

  // Calculate total commission amount and percentages
  const totalCommission = tenderData?.reduce(
  ...
  const percentages = tenderData?.map((row: any) => {
    const commissionAmt = (row.payments * row.commission) / 100 || 0;
    return totalCommission > 0 ? ((commissionAmt / totalCommission) * 100).toFixed(2) : 0;
  }) || [];
  ...
  const centerTextPlugin: Plugin<"doughnut"> = {
    ...
    beforeDraw(chart) {
      ...
      // Display only the total amount (no label, no decimals)
      ctx.font = "bold 18px Arial";
      ctx.fillStyle = "#000";
      ctx.fillText(
        `$${Math.round(totalCommission).toLocaleString()}`,
        centerX,
        centerY // Center the value vertically
      );
      ctx.restore();
    },
  };
    ...
    <main>
      ...
      <div className="below-lg:hidden tablet:hidden below-md:w-[430px] below-md:h-[430px]">
        <Doughnut
          data={data}
          options={options}
          plugins={isMobile ? [centerTextPlugin] : [centerTextPlugin, customArrowsPlugin]}
        />
      </div>
    </main>
  );
